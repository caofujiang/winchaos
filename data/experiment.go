...
var insertExpDML = `INSERT INTO
	experiment (uid, command,cmd_type,sub_command, flag, status, error, create_time, update_time)
	VALUES (?, ?, ?, ?, ?, ?, ?, ?,?)
`
...
func getExperimentModelsFrom(rows *sql.Rows) ([]*ExperimentModel, error) {
	...
	for rows.Next() {
		var id int
		var uid, command, cmdType, subCommand, flag, status, error, createTime, updateTime string
		err := rows.Scan(&id, &uid, &command, &cmdType, &subCommand, &flag, &status, &error, &createTime, &updateTime)
		if err != nil {
			return nil, err
		}
